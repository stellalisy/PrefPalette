...
class SFTDataset(Dataset):
    ...
    def process_data(self, data):
        ...
        if not self.pretrain_mode:
            ...
            response_ids_len = response_token["attention_mask"].int().sum().item()
            prompt_token = self.tokenizer(
                prompt,
                max_length=max(0, self.max_length-response_ids_len),
                padding=False,
                truncation=True,
                return_tensors="pt",
                add_special_tokens=False,
            ...
            if not prompt or not response: #s or prompt_ids_len >= self.max_length - 2:
                prompt = None
        else:
            prompt_ids_len = 0

        return {"prompt": prompt, "response": response, "prompt_ids_len": prompt_ids_len, "id": data.get("id", None)}
    ...
    def __getitem__(self, idx):
        ...
        info = {"input": prompt, "output": response, "input_length": input_token["attention_mask"].int().sum().item(), "id": sample_id}
